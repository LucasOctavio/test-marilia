...
from ..schemas.pergunta_schema import PerguntaCreate, PerguntaResponse
...
@pergunta_router.post("/create", response_model=PerguntaResponse, status_code=201)
def criar_pergunta_endpoint(pergunta_input: PerguntaCreate, db = Depends(get_session)):
    return criar_pergunta(pergunta_input, db)

@pergunta_router.put("/update/{pergunta_id}", response_model=PerguntaResponse)
